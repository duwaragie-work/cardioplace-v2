...
//
// Under `output: 'export'`, an in-app `router.push('/alerts?id=<ULID>')` makes
// Next fetch the RSC payload `/alerts.txt?id=<ULID>&_rsc=…` from the host — so
// the alert ULID lands in the CDN access log on EVERY click. Fix: in-app clicks
// stash the id in sessionStorage and navigate to the BARE route (no query); the
// shell reads it on mount. A `?id=` fallback is kept on the shell so external
// email/push deep-links still resolve.
//
...
/** Fired after a stash so a shell already mounted on the SAME bare route can
 *  re-read and update (a same-URL push doesn't remount). */
