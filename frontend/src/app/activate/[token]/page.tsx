...
// Invite-activation landing page (patient app).
//
// Email link → /activate/:token (rawToken hash lives server-side as
// UserInvite.tokenHash). Two-step UX:
//   1. GET /api/v2/auth/invite/:token   — render "Activate as Jane, Patient
//                                          at Cedar Hill" before the click.
//   2. POST /api/v2/auth/invite/:token/accept — claim the invite, set
//                                          HttpOnly session cookies, then
//                                          login() + navigate.
//
...
export default function ActivateInvitePage({ params }: PageProps) {
  ...
  async function handleActivate() {
    ...
    try {
      ...
      login(data);
      // Invite activation bypasses the onboarding + privacy gate (backend
      // pre-completes onboardingStatus and logs the policy acknowledgement
      // on the audit trail), so the patient lands straight on /dashboard.
