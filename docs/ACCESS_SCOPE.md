...
- **AccessLog audit write** — every PHI read/write on the seven PHI models writes an audit row (actor, action, model, recordId, IP, UA, timestamp) per HIPAA §164.312(b). Implemented in the Prisma client extension (`backend/src/common/prisma-extensions/access-log.extension.ts` + `CardioplaceClsModule` interceptor). Ships in PR #115 (2026-07-01).
